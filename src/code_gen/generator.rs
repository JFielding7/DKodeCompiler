...
use crate::types::builtin_type::BuiltinType::String;
...
impl<'llvm_ctx> CodeGenerator<'llvm_ctx> {
    ...
    fn emit_string_literal(&mut self, literal: DefaultSymbol) -> Value<'llvm_ctx> {
        let str_data_type_id = self.compiler_context.type_arena.builtin_type_id(String);

        let str_new_symbol = self.compiler_context.string_interner
            .get_intern_symbol("str_new");
        ...
        let str_const = self.llvm_context.const_string(literal_str.as_bytes(), true);
    ...
    fn add_builtin_functions(&mut self) {
        let str_type_id = self.compiler_context.type_arena.builtin_type_id(String);
        let str_type = &self.data_type_map.get(&str_type_id).unwrap().data_type;
        let str_llvm_type: BasicTypeEnum = str_type.into();

        let fn_type = str_llvm_type.fn_type(
            &[self.llvm_context.ptr_type(AddressSpace::default()).into(), self.llvm_context.i64_type().into()],
            false,
        );

        let str_new_name = "str_new";
        let fn_value = self.module.add_function(str_new_name, fn_type, None);
        let str_new = self.compiler_context.string_interner
            .get_intern_symbol(str_new_name);

        let str_type = self.data_type_map.get_mut(&str_type_id).unwrap();
        str_type.add_method(str_new, fn_value);
    }
