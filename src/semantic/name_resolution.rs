...
use crate::semantic::error::SemanticError::UndefinedVariable;
...
impl<'a> NameResolver<'a> {
    ...
    fn handle_binary_operator(&mut self, op: &BinaryOperatorNode, wrapper_node: &ASTNode) -> SemanticResult<()> {
        if op.op_type.precedence_group() == Assign {
            let left_node = self.ast.lookup(op.left);
            if let Variable(var) = &left_node.node_type {
                self.ctx.symbol_table.insert(var.name, wrapper_node.span, wrapper_node.scope_id);
            }
        }

        if op.op_type.is_left_assoc() {
    ...
    fn resolve_for(&mut self, for_node: &ForNode) -> SemanticResult<()> {
        let item_var_node = self.ast.lookup(for_node.item_variable);

        if let Variable(item_var) = &item_var_node.node_type {
            self.ctx.symbol_table.insert(item_var.name, item_var_node.span, item_var_node.scope_id);
        } else {
            unreachable!("Expected for loop item to be a variable")
        }
    ...
    fn resolve_statement_names(&mut self, statement_root_id: ASTNodeId) -> SemanticResult<()> {
        ...
        match &node.node_type {
            ...
            BinaryOperator(op) => self.handle_binary_operator(op, node)?,
    ...
    fn resolve_statements(&mut self) -> SemanticResult<()> {
        for statement_nodes in self.ast.statement_root_node_id_iter() {
            self.resolve_statement_names(*statement_nodes)?;
        }
    ...
    pub fn resolve(ast: &'a AST, ctx: &'a mut CompilerContext) -> SemanticResult<()> {
        NameResolver::new(ast, ctx).resolve_statements()
    }
}
