...
use crate::lexical_analysis::lexical_analysis;
use crate::syntax_analysis::{syntax_analysis, SyntaxAnalysisOutput};
...
fn compile_source_file(source_file: &SourceFile, compiler_context: &mut CompilerContext) -> CompilerResult<()> {
    ...
    let syntax_output: SyntaxAnalysisOutput = syntax_analysis(source_lines)?;

    let annotated_ast = semantic_analysis(syntax_output, compiler_context)?;
