...
use crate::ast::access_node::MemberType::Method;
...
use crate::semantic_analysis::error::SemanticError::{DuplicateClassFieldName, DuplicateFunctionName, DuplicateParameterName, InvalidLValue, UndefinedVariable};
...
impl<'ast> NameResolver<'ast> {
    ...
    fn resolve_access(&mut self, access: &AccessNode) -> CompilerResult<()> {
        ...
        if let Method(args) = &access.member.member_type {
            for &arg_expr_id in args {
                self.resolve_expression(arg_expr_id)?;
            }
        }
    ...
    fn resolve_function_signature(&mut self, func_def_node: &FunctionDefNode, span: SourceSpan) -> CompilerResult<()> {
        ...
        for (i, param) in func_def_node.params.iter().enumerate() {
            let inserted_symbol = self.symbol_table.insert_function_param(
    ...
    fn resolve_class_fields(&mut self, class_def_node: &ClassDefNode) -> CompilerResult<()> {
        ...
        for (i, field) in class_def_node.fields.iter().enumerate() {
            ...
            if !inserted {
                return Err(DuplicateClassFieldName(field.name).at(field.span));
            }
        }
    ...
    fn resolve_items(&mut self, items: &Vec<ItemId>) -> CompilerResult<()> {
        for &item_id in items {
            ...
            match &item.node_type {
                ...
                ClassDef(class_def_node) => {
                    self.resolve_class_fields(class_def_node)?;
                }
            }
        }
